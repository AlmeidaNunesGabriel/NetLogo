...
public class SimulationAppState extends BaseAppState {
    ...
    // O valor inicial que você definiu.
    ...
    // [ADICIONADO] Métodos para alterar o número de loops dinamicamente.
    ...
    public void decreaseMaxLoops(int amount) {
        this.maxLoopsSetting = Math.max(1, this.maxLoopsSetting - amount); // Garante que não seja menor que 1
        if (simulationEngineRef != null) {
            simulationEngineRef.updateMaxLoopsSetting(this.maxLoopsSetting);
        }
    }

    // O restante da sua classe continua aqui...
    // ... (cole o resto da sua classe SimulationAppState aqui, não precisa mudar mais nada nela)
    // ...
    // --- Getters e Setters ---
